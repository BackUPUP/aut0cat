...
def check1(name, amount, id):
    ...
    url = f"https://api-gateway.toss.im:11099/api-public/v3/cashtag/transfer-feed/received/list?inputWord={id}"
    ...
    for transaction in data:
        ...
        if name == transfer_name: # 1. 이름이 동일한가?
            with open("확인된거래.txt") as f: # 입금자명을 정해준다면 이미 처리된 거래인지 확인하지 않아도 된다.
                checked = f.read()

            if not str(transfer_id) in checked and amount == transfer_amount: # 2. 이전에 처리되지 않은 기록인가? / 3. 금액이 동일한가?
